...
export const library = [
  ...
  { id: "astral-comet-ace", type: "monster", name: "天穹逆星者", element: "light", stars: 5, atk: 2300, def: 1800, icon: "逆", text: "需要 1 只我方场上怪兽作为祭品才能通常召唤。攻击后自身攻击力提升 200；也可由卡牌效果特殊召唤。", tributeCost: 1, afterAttack: "grow200" },
  ...
  { id: "void-siege-breaker", type: "monster", name: "虚痕镇压者", element: "shadow", stars: 5, atk: 2600, def: 1200, icon: "镇", text: "需要 1 只我方场上怪兽作为祭品才能通常召唤。高攻击力会逼迫对手交出守护资源。", tributeCost: 1 },
...
export const scenarioSetups = {
  ...
  trioTributeSummon: {
    label: "三曜祭品召唤测试",
    difficulty: "demo",
    text: "起手提供三只低阶怪兽和曜冕裁决者，用来验证终局三曜神卡不能无祭品通常召唤。",
    goal: "选择曜冕裁决者，献祭三只我方场上怪兽后完成三曜祭品召唤。",
    objectives: ["查看曜冕裁决者的三祭品需求。", "依次选择三只我方场上怪兽作为祭品。", "确认三曜神卡登场且三只祭品进入墓地。"],
    hints: ["三曜神卡都需要正好 3 只祭品才能通常召唤。", "融合召唤和卡牌效果特殊召唤不使用这次通常召唤的祭品选择。"],
    playerHand: ["trio-sun-judicator", "war-chant"],
    playerField: ["spark-runner", "lumen-gearlet", "ember-soul-initiate"],
    playerDeck: ["solar-knight"],
    aiField: ["iron-guardian"],
    aiHand: [],
  ...
  ace: {
    label: "王牌攻势",
    text: "场上预置一只低阶怪兽，起手给熔核巨像和强化卡，方便验证一祭品王牌与攻击特写。",
    goal: "献祭新星侍从召唤熔核巨像，再攻击对手怪兽并观察王牌攻势特写动画。",
    playerHand: ["flare-titan", "war-chant", "battle-trance", "seer-call", "mirror-snare"],
    playerField: ["nova-squire"],
    aiField: ["iron-guardian"],
  ...
  trioBackrowCounterPlanning: {
    ...
    adaptiveBossCounter: true,
    adaptiveBossPhase: 1,
    text: "规则测试场景：玩家本回合新盖一张陷阱，日曜只知道卡位而不知道卡面；AI 后场仅剩一个空位。",
    goal: "盖放日冕诱锁并结束回合；观察 AI 优先盖放断链裁决，并让日曜锁定刚刚建立的卡位。",
    playerLp: 9000,
    aiLp: 4000,
    playerHand: ["trio-solar-snare"],
    playerDeck: ["guard-sigil"],
    playerField: [],
    aiField: ["trio-sun-judicator"],
    aiTraps: ["guard-sigil", "summon-flare", "weakening-web", "void-lock"],
    aiHand: ["mirror-snare", "chain-nullifier"],
    aiDeck: ["guard-sigil"],
    aiStyle: "scriptedPressure"
  },
  trioRushFinalePlanning: {
    label: "三曜抢攻终结",
    setupVisibility: "internal",
    adaptiveBossCounter: true,
    adaptiveBossPhase: 3,
    ...
    aiHand: ["star-shield", "battle-trance"],
    aiDeck: ["guard-sigil"],
    aiStyle: "scriptedPressure"
  },
  trioTrapPlanning: {
  ...
  trioChainLifecycle: {
    label: "三曜连锁生命周期",
    text: "规则测试场景：日冕诱锁响应日曜攻击，断链裁决随后反制，用来验证连锁中的盖牌与生命值显示。",
    goal: "结束回合并发动日冕诱锁；连锁结算前卡牌应留在魔陷区，结算后生命值应稳定显示为 850。",
    playerLp: 3050,
    aiLp: 4000,
    playerHand: [],
    playerDeck: ["guard-sigil", "mirror-snare"],
    playerField: ["spark-runner"],
    playerTraps: ["trio-solar-snare"],
    aiField: ["trio-sun-judicator"],
    aiTraps: ["chain-nullifier"],
    aiHand: [],
    aiDeck: ["mirror-snare"],
  ...
  protagonistTrioOmegaFinale: {
    ...
    aiStyle: "scriptedPressure",
    objectives: [
      "太阳神第一击会锁定最前方的星线护续；它提前离场时，攻击后破坏不会转移到日冕诱锁。",
      "可放弃响应让星线护续承受破坏，也可发动它与断链裁决交换；两条路线都会保住后方诱锁。",
    ...
    recommendedLine: [
      "第一回合：盖下星线护续（前）与日冕诱锁（后），让太阳神先锁定星线护续。",
      "可选择不发动，让星线护续承受攻击后破坏；也可发动它与断链交换，锁定不会转移——日冕诱锁都会保住。",
    ...
    storyBeats: [
      ...
      },
      {
        ...
        speaker: "ai",
        line: "断链裁决封住了你的陷阱——但太阳神的锁定不会转移，日冕诱锁仍能留到第二击！"
      },
      {
  ...
  protagonistTrioOmegaFinaleRush: {
    ...
    goal: "沿用终焉篇的破解框架：让第一击锁定前方星线护续，选择承受破坏或用它与断链交换以保住日冕诱锁，再以解印射线拆断链，回召小卫发动三曜终断；但月曜帷幕可能在任意回合压制小卫——进攻前先解除压制，再用防线斩落最后一尊太阳神。",
    difficulty: "challenge",
    aiStyle: "scriptedPressure",
    objectives: [
      "太阳神第一击会锁定最前方的星线护续；它提前离场时，攻击后破坏不会转移到日冕诱锁。",
    ...
    recommendedLine: [
      "第一回合盖下星线护续（前）与日冕诱锁（后），让太阳神先锁定星线护续；可不发动，也可用它换掉断链。",
