...
test("current chain stack exposes artwork, owner, and structured resolution order", () => {
  assert.match(trapResponseRenderer, /heading\.className = "chain-stack-head"/);
  assert.match(trapResponseRenderer, /applyCardArt\(art, entry\.cardId\)/);
  assert.match(trapResponseRenderer, /row\.dataset\.owner = entry\.owner/);
  assert.match(trapResponseRenderer, /chain-resolution-step/);
